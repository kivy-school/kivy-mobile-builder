...
PEP 425 platform string like ``android_24_arm64_v8a``. Each wheel ships the
...
# Vendored from kivy-school/ksproject (scripts/build_python_target.py and
# src/ksproject_utils/gradle/cpython_android.py). Both live side by side here,
# so import the helper from this script's own directory.
SCRIPT_DIR = Path(__file__).resolve().parent
REPO_ROOT = SCRIPT_DIR.parents[1]
sys.path.insert(0, str(SCRIPT_DIR))

from cpython_android import (  # noqa: E402
    ANDROID_VERSION,
    _TRIPLES,
    android_prefix,
    install_cpython_android,
...
# Android API level the runtime is compiled against. Must be >= 24: bionic
# only exports if_nameindex()/if_freenameindex() from API 24 (net/if.h marks
# them __INTRODUCED_IN(24)), and CPython gates socket.if_nametoindex,
# socket.if_indextoname and socket.if_nameindex behind that single
# HAVE_IF_NAMEINDEX configure check. Building at 21 (CPython 3.13's
# android-env.sh default; 3.14 already moved to 24) silently drops all
# three, along with getgrgid_r/getgrnam_r and preadv/pwritev.
...
def build_wheel(
    ...
    wheel_meta = (
        f"Wheel-Version: 1.0\n"
        f"Generator: kivy-mobile-builder build_python_target.py\n"
...
def main() -> int:
    ...
    ap.add_argument(
        ...
        help="min Android API level to compile against and tag the wheel with "
        "(default: 24)",
    )
    ap.add_argument(
        "--work-dir",
    ...
    # CPython's Android/android-env.sh picks the clang target triple from
    # ANDROID_API_LEVEL (default 21 on 3.13, 24 on 3.14+). Pin it to --api so
    # the compiled binary and the wheel's platform tag can never drift apart.
    os.environ["ANDROID_API_LEVEL"] = str(args.api)
